        ...
        <div className="economy-head">
          <div>
            ...
            <h2 className="display display-md" style={{ marginTop: 14 }}>
              Money that doesn't sleep,<br />
              for a country that <em>doesn't either.</em>
            </h2>
          </div>
          <p className="lede economy-lede">
            Ghana's 24-hour economy agenda needs financial rails that match it. Bank cut-offs, weekends, and public holidays are friction that traders, exporters, and the diaspora can't afford. GHST settles in seconds, every second of the year.
          </p>
        </div>
...
// ── Timeline infographic: traditional banking vs GHST ──
function Timeline24() {
  const hours = Array.from({ length: 25 }, (_, i) => i); // 0..24

  // Compute live time-of-day marker position (0..1)
  const [tNow, setTNow] = useStateE(() => {
    const d = new Date();
    ...
    <div className="timeline24">
      <div className="timeline24-head">
        ...
        <div className="timeline24-legend">
          <span><span className="legend-swatch legend-bank" />Traditional banks</span>
          <span><span className="legend-swatch legend-ghst" />GHST</span>
        </div>
      </div>
      ...
      <div className="timeline-row">
        ...
        <div className="timeline-track">
          <div className="timeline-bar timeline-bar--bank" style={{ left: `${(8.5/24)*100}%`, width: `${(7.5/24)*100}%` }}>
        ...
        <div className="timeline-scale-track">
          {hours.map(h => {
            ...
            return (
              <span
                key={h}
                className={`tick ${major ? 'tick-major' : ''}`}
                style={{ left: `${(h/24)*100}%` }}
              >
...
function UseCase({ visual, tag, title, body, stat, statLabel }) {
  return (
...
// ── Infographic icons (corporate, abstract) ──
function MarketIcon() {
  return (
...
function WeekendIcon() {
  // 7-day strip with Sat/Sun lit up
      ...
      {days.map((d, i) => {
        ...
        return (
          <g key={i}>
            <rect
              x={x} y={20}
              width="18" height="40" rx="4"
              fill={isWeekend ? '#785AFF' : '#ebe5ff'}
            />
            <text x={x+9} y={45} textAnchor="middle" fontSize="11" fontWeight="600"
              fill={isWeekend ? '#fff' : '#785AFF'}>{d}</text>
            {isWeekend && (
...
function DiasporaIcon() {
  // Two cities with arc connecting; clock-style
  return (
    <svg viewBox="0 0 200 80" width="100%" height="100%">
      ...
      {/* arc */}
      <path d="M 30 60 Q 100 -10, 170 60"
        fill="none" stroke="url(#arc)" strokeWidth="2" strokeDasharray="2 4" />
      {/* moving dot along arc — uses Bezier path-following via animateMotion */}
      ...
      <g transform="translate(30 60)">
        ...
        <text textAnchor="middle" y="3" fontSize="9" fontWeight="600" fill="#0a0822">LDN</text>
      </g>
      {/* destination city */}
      <g transform="translate(170 60)">
        <circle r="10" fill="#785AFF" />
        <text textAnchor="middle" y="3" fontSize="9" fontWeight="600" fill="#fff">ACC</text>
      </g>
      {/* tiny clock label */}
